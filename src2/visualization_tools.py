...
def plot_spatial_graph(data, title=None):
    ...
    # runner true position
    runner_position = data.runner_position.cpu().numpy().reshape(-1)
    
    pos = data.x[:, :2].cpu().numpy()
    # restore runner position
    pos[runner_idx] = runner_position
...
def plot_optimal_runs(run, tracking_frame_groups,player_to_team, absolute_paths, plot_ball = True,plot_defense = True):
    ...
    for run_type in absolute_paths:
        absolute_path = absolute_paths[run_type]  
