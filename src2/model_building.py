...
def build_graph_from_frame(frame_df, runner_player_id, player_to_team):
    ...
    ## Need to zero out the runner position values for graph so the model doesn't know the actual run path
    runner_row_idx = frame_df.index[
        frame_df["player"] == runner_player_id
    ][0]
    runner_zero_cols = [
        "x", "y",
        "dx", "dy",
    ...
    runner_position = torch.tensor(frame_df.loc[runner_row_idx,["x","y"]],dtype=torch.float)
    frame_df.loc[runner_row_idx, runner_zero_cols] = 0.0
...
class TemporalRunnerDataset(Dataset):
    ...
    """
    def __init__(self, tracking_df, run_features, player_to_team, run_type_to_int):
        self.df = tracking_df
        self.run_features = run_features
        self.player_to_team = player_to_team
        self.run_type_to_int = run_type_to_int
    ...
    def __getitem__(self, idx):
        ...
        shot_label = int(run['possession_lead_to_shot'])
        goal_label = int(run['possession_lead_to_goal'])
        run_type_id = int(self.run_type_to_int[run['event_subtype']])

        return graphs, target_path, shot_label, goal_label, run_type_id, start_pos
...
def collate_fn(batch, max_len=100):
    ...
    """
    graphs_list, targets_list, shot_labels_list, goal_labels_list, run_type_list, start_pos_list = zip(*batch) 
    ...
    shot_labels = torch.tensor(shot_labels_list, dtype=torch.int)
    goal_labels = torch.tensor(goal_labels_list, dtype=torch.int)
    run_type_labels = torch.tensor(run_type_list, dtype=torch.int)
    start_pos = torch.tensor(start_pos_list, dtype=torch.float)

    return batch_graphs, padded_targets, lengths, shot_labels, goal_labels, run_type_labels, start_pos
...
class TemporalRunnerGNN(nn.Module):
    ...
    def __init__(self, node_feat_dim, edge_dim, gnn_hidden_dim,num_run_types):
        super().__init__()

        self.gnn1 = TransformerConv(
            in_channels=node_feat_dim + gnn_hidden_dim,
        ...
        self.global_attention = GlobalAttention(self.att_mlp)
        
        self.run_type_embed = nn.Embedding(num_run_types, gnn_hidden_dim)
        
        self.start_pos_mlp = nn.Sequential(
            nn.Linear(2, gnn_hidden_dim),
            nn.ReLU(),
        ...
        self.intent_gate = nn.Parameter(torch.tensor(0.5))
    ...
    def forward(self, graphs_batch, lengths, run_types, start_pos):#, run_types
        ...
        # Unpack graph
        # --------------------------------------------------
        x = graphs_batch.x                          # [num_nodes, F]
        edge_index = graphs_batch.edge_index
        edge_attr = graphs_batch.edge_attr
        batch_vec = graphs_batch.batch              # [num_nodes] → frame index

        edge_attr = edge_attr / edge_attr.std(dim=0, keepdim=True).clamp(min=1e-6)

        # --------------------------------------------------
        # Build run-level intent
        # --------------------------------------------------
        run_type_embed = self.run_type_embed(run_types)     # [B, D]
        start_pos_embed = self.start_pos_mlp(start_pos)     # [B, D]
        run_context = run_type_embed + start_pos_embed      # [B, D]

        # --------------------------------------------------
        # Expand run → frame
        # --------------------------------------------------
        frame_context = torch.cat([
            run_context[i].repeat(lengths[i], 1)
        ...
        # Expand frame → node
        # --------------------------------------------------
        node_context = frame_context[batch_vec]             # [num_nodes, D]

        # --------------------------------------------------
        # Inject intent at NODE LEVEL
        # --------------------------------------------------
        x = torch.cat([x, node_context], dim=-1)             # [num_nodes, F + D]
        ...
        # Pool per frame
        # --------------------------------------------------
        # graph_embeds = self.graph_pool(x, batch_vec)         # [num_frames, D]
        graph_embeds = self.global_attention(x, batch_vec)
        ...
        graph_embeds_per_run = []
        start = 0
        for length in lengths:
            run_embeds = graph_embeds[start:start + length]
            start += length
            ...
            if length < max_len:
                ...
                run_embeds = torch.cat([run_embeds, padding], dim=0)

            graph_embeds_per_run.append(run_embeds)

        graph_embeds_batch = torch.stack(graph_embeds_per_run)  # [B, T, D]

        # --------------------------------------------------
        # Temporal encoder
        # --------------------------------------------------
        T = graph_embeds_batch.size(1)
        x = graph_embeds_batch + self.temporal_pe[:, :T]
        ...
        # Output: dx, dy
        # --------------------------------------------------
        pred_path = self.pos_head(out)   # [B, T, 2]
        return pred_path
    
    
def train_model(model, device, dataloader,num_epochs = 20,output_file = "temporal_runner_gnn_intent.pth"):
    """
    ...
    for epoch in range(num_epochs):
        ...
        total_loss = 0
        for batch_graphs, padded_targets, lengths, shot_labels, goal_labels, run_type_labels, start_pos in tqdm(dataloader):
            ...
            batch_vec = batch_graphs.batch  # [num_nodes], frame indices for nodes
            if sum(lengths) != (batch_vec.max().item() + 1):
                # print(f"Skipping batch: sum(lengths)={sum(lengths)} but batch_vec.max()={batch_vec.max().item()}")
                continue 
            
            optimizer.zero_grad()

            pred_path = model(batch_graphs, lengths,run_type_labels, start_pos) 
...
def predict_optimal_run(run,model,tracking_frame_groups,device,player_to_team,run_type_to_int, run_type_to_predict):
    """
    ...
    run_type_id = run_type_to_int[run_type]
    frame_start = int(run['frame_start_run'])
    frame_end = int(run['frame_end_run'])
    ...
    run_type_id = torch.tensor(
        [run_type_to_int[run_type_to_predict]],
        dtype=torch.long,
        device=device
    )  # [1]
    ...
    with torch.no_grad():
        pred_path = model(graphs_batch, lengths, run_type_id,start_pos)  
...
def evaluate_all_runs(runs_to_predict, model, tracking_frame_groups, device, player_to_team,run_type_to_int):
    """
    ...
    for idx, run in runs_to_predict.iterrows():
        # Predict run path
        try:
            absolute_path = predict_optimal_run(run, model, tracking_frame_groups, device, player_to_team,run_type_to_int)
        except:
            pass
